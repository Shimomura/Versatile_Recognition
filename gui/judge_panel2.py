...
class JudgePanel(base.ContentsPanelBase):
    ...
    def __init__(self,parent,id):
        ...
        self.name_text = wx.TextCtrl(self, wx.ID_ANY, pos=(10,115), size=(450,30))#change
        self.name_text.SetFont(font)#change
        ...
        self.Bind(wx.EVT_BUTTON, self.choose_file,self.button_study)#add        

    def get_studied_list(self):
        return ["猫","犬","花"]

    def choose_file(self, evt):#add
        dialog = wx.FileDialog(self, style=wx.FD_CHANGE_DIR, message="ファイルを選択してください")
        if dialog.ShowModal() == wx.ID_OK:
            self.folder = dialog.GetPath()
        dialog.Destroy()
        self.name_text.SetValue(self.folder)
